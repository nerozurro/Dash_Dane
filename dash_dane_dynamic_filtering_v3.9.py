...
# values = {
#     'value-1': {'procedencia_departamento', 'destino_ciudad_mercado'},
#     'value-2': {'procedencia_municipio', 'destino_ciudad_mercado'},
#     'value-3': {'grupo', 'destino_ciudad_mercado'},
#     'value-4': {'alimento', 'destino_ciudad_mercado'},
#     'value-5': {'procedencia_departamento', 'grupo', 'destino_ciudad_mercado'},
#     'value-6': {'procedencia_departamento', 'alimento', 'destino_ciudad_mercado'},
#     'value-7': {'procedencia_municipio', 'grupo', 'destino_ciudad_mercado'},
#     'value-8': {'procedencia_municipio', 'alimento', 'destino_ciudad_mercado'},
#     'value-9': {'procedencia_departamento', 'grupo'},
#     'value-10': {'procedencia_departamento', 'alimento'},
#     'value-11': {'procedencia_municipio', 'grupo'},
#     'value-12': {'procedencia_municipio', 'alimento'},
#     'value-13': {'procedencia_departamento'},
#     'value-14': {'procedencia_municipio'},
#     'value-15': {'grupo'},
#     'value-16': {'alimento'},
#     'value-17': {'destino_ciudad_mercado'}

# }

# optionsGroup = [
#     {'label': 'procedencia departamento -> destino mercado', 'value': 'value-1'},
#     {'label': 'procedencia municipio -> destino mercado', 'value': 'value-2'},
#     {'label': 'grupo -> destino mercado', 'value': 'value-3'},
#     {'label': 'alimento -> destino mercado', 'value': 'value-4'},
#     {'label': 'procedencia departamento -> grupo -> destino_ciudad_mercado', 'value': 'value-5'},
#     {'label': 'procedencia departamento -> alimento -> destino_ciudad_mercado', 'value': 'value-6'},
#     {'label': 'procedencia municipio -> grupo -> destino_ciudad_mercado', 'value': 'value-7'},
#     {'label': 'procedencia municipio -> alimento -> destino_ciudad_mercado', 'value': 'value-8'},
#     {'label': 'procedencia departamento -> grupo', 'value': 'value-9'},
#     {'label': 'procedencia departamento -> alimento', 'value': 'value-10'},
#     {'label': 'procedencia municipio -> grupo', 'value': 'value-11'},
#     {'label': 'procedencia municipio -> alimento', 'value': 'value-12'},
#     {'label': 'procedencia departamento', 'value': 'value-13'},
#     {'label': 'procedencia_municipio', 'value': 'value-14'},
#     {'label': 'grupo', 'value': 'value-15'},
#     {'label': 'alimento', 'value': 'value-16'},
#     {'label': 'destino mercado', 'value': 'value-17'}
# ]
global df_grouped
global function_agg
#df_grouped=df.copy()

df_grouped = df.groupby(['procedencia_departamento', 'destino_nombre_mercado'])[['cantidad_kg']].agg('sum')
...
print('')
print('Starting Web App Service :D .....')
...
html.Br(),
    ...
    dbc.Row([  # Tittle
        dbc.Col(html.H1("Dashboard Mercados Colombia",
...
# INICIA SERIE DE TIEMPO
    ...
    dbc.Row([  # Tittle
        dbc.Col(html.H3("SERIES DE TIEMPO",
                        className='text-center text-primary, mb-4'),
                width=12)
    ], no_gutters=False, justify='around', align='center'),
...
# INICIA MAPA
    ...
    dbc.Row([  # Tittle
        dbc.Col(html.H3("ANÁLISIS GEO ESPACIAL",
                        className='text-center text-primary, mb-4'),
                width=12)
    ], no_gutters=False, justify='around', align='center'),
...
# INICIA GROUP BY TABLE
    ...
    dbc.Row([  # Tittle
        dbc.Col(html.H3("TABLA AGRUPADORA",
                        className='text-center text-primary, mb-4'),
                width=12)
    ], no_gutters=False, justify='around', align='center'),
...
def update_time_series(time_series_selection, sl, dpt, mpio, gr, alim, merD):
    ...
    df_grouped_time = df_filtrado.groupby(time_series_selection)[[column_agg]].agg(function_agg)
    df_grouped_time = df_grouped_time.sort_values(by=column_agg, ascending=False)
    ...
    df_grouped_time = df_grouped_time.groupby(columns_plot_time)[[column_agg]].agg(function_agg)
...
def update_map(map_bubble_selection, sl, dpt, mpio, gr, alim, merD):
    ...
    if  (map_bubble_selection=='opMuO'): #municipio origen
        ...
        df_mapa = df_filtrado.groupby(columns_procedencia)[[column_agg]].agg(function_agg)
    ...
    if  (map_bubble_selection=='opMeD'): #  MERCADO DESTINO
        ...
        df_mapa = df_filtrado.groupby(columns_destino)[[column_agg]].agg(function_agg)
        df_mapa.reset_index(inplace=True)
        df_mapa = pd.merge(df_mapa, df_coor_merc, how="left", left_on=['destino_ciudad_mercado'], right_on=['destino_ciudad_mercado'])
    ...
    if  (map_bubble_selection=='opMuOMuD'): # Municipio origen a Municipio Destino
        ...
        df_mapa2 = df_mapa2.groupby(columns_destino)[[column_agg]].agg(function_agg)
        ...
        df_mapa2 = df_mapa2[['nombre', column_agg, 'LATITUD', 'LONGITUD', 'Ubicacion']]
    ...
    if  (map_bubble_selection=='opMuOMeD'): # Municipio Origen a MercadoDestino
        ...
        df_mapa2 = df_filtrado.groupby(columns_destino)[[column_agg]].agg(function_agg)
        df_mapa2.reset_index(inplace=True)
        df_mapa2 = pd.merge(df_mapa2, df_coor_merc, how="left", left_on=['destino_ciudad_mercado'], right_on=['destino_ciudad_mercado'])
        df_mapa2 = df_mapa2.assign(Ubicacion="Destino")
        ...
        # print("MAPA DF CONCATENADO: ", df_mapa)


        df_mapa2 = df_filtrado[['procedencia_codigo_municipio', 'procedencia_municipio', 'destino_ciudad_mercado']]
        df_mapa2 = df_mapa2.groupby(['procedencia_codigo_municipio', 'procedencia_municipio', 'destino_ciudad_mercado']).count()
        df_mapa2.reset_index(inplace=True)
        df_mapa2 = pd.merge(df_mapa2, df_municipios_properties, how="left", left_on=["procedencia_codigo_municipio"], right_on=["MPIO_CDPMP"])
        df_mapa2 = pd.merge(df_mapa2, df_coor_merc, how="left", left_on=["destino_ciudad_mercado"], right_on=["destino_ciudad_mercado"])
        df_mapa2 = df_mapa2.dropna(subset=['MPIO_CDPMP'])
...
def update_group_table(agrupador1, agrupador2, agrupador3, agrupador4,
    ...
    df_grouped = df_filtrado.groupby(lista_agrupadores)[[column_agg]].agg(function_agg)
    ...
    # {:,.0f}
    print('succesfully calculated GroupBy table')
    print('')
    return [df_grouped.to_dict('records'), grouped_columns, styles]
...
def update_pivot_table(pivotSelector1, pivotSelector2,
    ...
    table = pd.pivot_table(df_filtrado, values=column_agg, index=[pivotSelector1], 
    ...
    df_grouped_pivot_graph = df_filtrado.groupby([pivotSelector1])[[column_agg]].agg(function_agg)
    df_grouped_pivot_graph = df_grouped_pivot_graph.sort_values(by=column_agg, ascending=False)
    ...
    df_grouped_pivot_graph = df_grouped_pivot_graph.groupby([pivotSelector1, pivotSelector2])[[column_agg]].agg(function_agg)
    df_grouped_pivot_graph.reset_index(inplace=True)
